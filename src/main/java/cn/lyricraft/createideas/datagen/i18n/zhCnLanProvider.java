...
public class zhCnLanProvider extends LanguageProvider {
    ...
    protected void addTranslations() {
        ...
        add(commonConfig.sectionTooltip("utility"), "一些新添加的实用物件及相关调整。");
        add(commonConfig.cv(CommonConfig.PORTABLE_STRESSOMETER), "便携式应力表");
        add(commonConfig.cvTooltip(CommonConfig.PORTABLE_STRESSOMETER), "允许合成和使用便携式应力表。");
        add(commonConfig.section("recipes"), "配方");
        add(commonConfig.sectionTooltip("recipes"), "管理添加的新配方以及对已有配方进行的调整。");
        add(commonConfig.cv(CommonConfig.BLAST_RECIPES_RAW_ORE_BLOCKS), "高炉熔炼粗矿块");
        add(commonConfig.cvTooltip(CommonConfig.BLAST_RECIPES_RAW_ORE_BLOCKS), "允许高炉直接熔炼粗铁块、粗金块和粗铜块，这也将使鼓风机批量冶炼可以处理这些粗矿块。");
        add(commonConfig.section("hauntingRecipes"), "批量缠魂配方");
        add(commonConfig.sectionTooltip("hauntingRecipes"), "新添加的鼓风机批量缠魂配方。");
        add(commonConfig.cv(CommonConfig.HAUNTING_RECIPE_AMETHYST_SHARD), "紫水晶碎片缠魂");
        add(commonConfig.cvTooltip(CommonConfig.HAUNTING_RECIPE_AMETHYST_SHARD), "启用紫水晶碎片批量缠魂转化为回响碎片的配方。");
        add(commonConfig.cv(CommonConfig.HAUNTING_RECIPE_BRICKS), "红砖块缠魂");
        add(commonConfig.cvTooltip(CommonConfig.HAUNTING_RECIPE_BRICKS), "启用红砖块批量缠魂转化为红色下界砖块的配方。");
        add(commonConfig.cv(CommonConfig.HAUNTING_RECIPE_LAPIS_BLOCK), "青金石块缠魂");
        add(commonConfig.cvTooltip(CommonConfig.HAUNTING_RECIPE_LAPIS_BLOCK), "启用青金石块批量缠魂转化为海晶石和海晶砂粒的配方。");
        add(commonConfig.section("bulkProcessingExtraExp"), "鼓风机批量处理的额外经验");
        add(commonConfig.sectionTooltip("bulkProcessingExtraExp"), "在进行鼓风机批量处理时，生成额外的经验颗粒作为产物。");
        add(commonConfig.cv(CommonConfig.BULK_BLASTING_EXTRA_EXP), "批量熔炼");
        add(commonConfig.cvTooltip(CommonConfig.BULK_BLASTING_EXTRA_EXP), "使鼓风机批量熔炼能生成经验颗粒。");
        add(commonConfig.cv(CommonConfig.BULK_SMOKING_EXTRA_EXP), "批量烟熏");
        add(commonConfig.cvTooltip(CommonConfig.BULK_SMOKING_EXTRA_EXP), "使鼓风机批量烟熏能生成经验颗粒。");
    }
}
